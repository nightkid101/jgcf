...
def gxlawyer_crawler():
    ...
    prefix_url = [{'url': 'http://www.gxlawyer.org.cn/cn/lawyer_honest?currentPageNo=1&', 'origin': '广西壮族自治区律师协会'}]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('广西壮族自治区律师协会 抓取URL：' + each_url)
        # get page count
        ...
        try:
            page_text = base_soup.find('body').find(attrs={"class": 'tzgg clearfix'}).find(
                attrs={"class": 'right'}).find('script').text
        ...
        for page_num in range(page_count):
            ...
            url = 'http://www.gxlawyer.org.cn/cn/lawyer_honest?currentPageNo=' + str(page_num + 1) + '&.html'
            ...
            try:
                ...
                all_li = page_soup.find('body').find(attrs={"class": 'tzgg clearfix'}).find(
                    attrs={"class": 'right'}).find_all('li')
                for index, each_li in enumerate(all_li):
                    title = each_li.find('a').text.strip()
                    href = each_li.find('a')['href'].strip()
                    true_url = urljoin(url, href)
                    publish_date = each_li.find('span').text.strip()
                    ...
                    if db.lawyers_data.find({'url': true_url}).count() == 0:
                        logger.info('广西壮族自治区律师协会新公告：' + true_url + ' title: ' + title)
                        post = {
                            'title': title,
    ...
    if len(result_list) > 0:
        logger.info('广西壮族自治区律师协会一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.lawyers_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('广西壮族自治区律师协会公告导入完成！')
        else:
            logger.error('广西壮族自治区律师协会公告导入出现问题！')
    else:
        logger.info('广西壮族自治区律师协会没有新公告！')
