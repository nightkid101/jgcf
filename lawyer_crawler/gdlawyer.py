...
logger = logger_init('广东省律师协会-数据抓取')
config = config_init()
...
def gdlawyer_crawler():
    result_list = []  # 用来保存最后存入数据库的数据
    prefix_url = [{'url': 'http://gdlawyer.org.cn/index.php?c=content&a=list&catid=189', 'origin': '广东省律师协会'}]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('广东省律师协会 抓取URL：' + each_url)
        # get page count
        ...
        try:
            page_count_text = base_soup.find(class_='pagination').text
            page_count = int(int(re.findall(r'\d+', page_count_text)[0]) / 10) + 1
        ...
        for page_num in range(page_count):
            ...
            try:
                ...
                all_li = page_soup.find_all(attrs={"class": 'container'})[2].find(attrs={"class": 'col-md-9'}).find_all(
                    attrs={"class": 'list-group-item'})
                for index, each_result in enumerate(all_li):
                    title = each_result.find_all('a')[-1]['title'].strip()
                    href = each_result.find_all('a')[-1]['href'].strip()
                    ...
                    if re.search(r'决定书', title):
                        publish_date = each_result.find_all('a')[-1].find('span').text.strip()
                        if db.lawyers_data.find({'url': true_url}).count() == 0:
                            logger.info('广东省律师协会新公告：' + true_url + ' title: ' + title)
                            post = {
                                'title': title,
    ...
    if len(result_list) > 0:
        logger.info('广东省律师协会一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.lawyers_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('广东省律师协会公告导入完成！')
        else:
            logger.error('广东省律师协会公告导入出现问题！')
    else:
        logger.info('广东省律师协会没有新公告！')
