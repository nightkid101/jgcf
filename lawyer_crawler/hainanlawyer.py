...
logger = logger_init('海南省律师协会-数据抓取')
config = config_init()
...
def hainanlawyer_crawler():
    result_list = []  # 用来保存最后存入数据库的数据
    prefix_url = [{'url': 'http://www.hainanlawyer.org/index.php?c=content&a=list&catid=21', 'origin': '海南省律师协会'}]
    for each_url_info in prefix_url:
        ...
        try:
            page_count_text = base_soup.find(id='pages').find_all('a')[-1].attrs['href']
        ...
        for page_num in range(page_count):
            ...
            try:
                ...
                all_li = page_soup.find(attrs={"class": 'con_news'}).find_all('li')
    ...
    if len(result_list) > 0:
        logger.info('海南省律师协会一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.lawyers_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('海南省律师协会公告导入完成！')
        else:
            logger.error('海南省律师协会公告导入出现问题！')
    else:
        logger.info('海南省律师协会没有新公告！')


if __name__ == "__main__":
    hainanlawyer_crawler()
