...
logger = logger_init('贵州省律师协会-数据抓取')
config = config_init()
...
def gzsls_crawler():
    ...
    prefix_url = [{'url': 'http://www.gzsls.com/ArticleList.aspx?ClassCode=YJDT', 'origin': '贵州省律师协会'}]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('贵州省律师协会 抓取URL：' + each_url)
        # get page count
        ...
        try:
            page_count_text = base_soup.find(class_='list').find(class_='fy').text
        ...
        for page_num in range(page_count):
            ...
            url = each_url + '&pi=' + str(page_num + 1)
            ...
            try:
                ...
                for index, each_result in enumerate(all_li):
                    ...
                    publish_date = re.search(r'(.{4}-.{2}-.{2})', each_result.text).group(1).strip()
                    ...
                    if re.search(r'(处分决定书|行政处罚)', title):
                        if db.lawyers_data.find({'url': true_url}).count() == 0:
                            logger.info('贵州省律师协会新公告：' + true_url + ' title: ' + title)
                            post = {
                                'title': title,
    ...
    if len(result_list) > 0:
        logger.info('贵州省律师协会一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.lawyers_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('贵州省律师协会公告导入完成！')
        else:
            logger.error('贵州省律师协会公告导入出现问题！')
    else:
        logger.info('贵州省律师协会没有新公告！')
