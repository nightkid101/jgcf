...
logger = logger_init('深圳市律师协会-数据抓取')
config = config_init()
...
def szlawyers_crawler():
    result_list = []  # 用来保存最后存入数据库的数据
    prefix_url = [{'url': 'http://www.szlawyers.com/integrity', 'origin': '深圳市律师协会'}]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('深圳市律师协会 抓取URL：' + each_url)
        # get page count
        ...
        try:
            text = base_soup.find(attrs={"class": 'page'}).find('script').text
            ...
            try:
                ...
                for index, each_result in enumerate(all_li):
                    ...
                    if db.lawyers_data.find({'url': true_url}).count() == 0:
                        logger.info('深圳市律师协会新公告：' + true_url + ' title: ' + title)
                        post = {
                            'title': title,
    ...
    if len(result_list) > 0:
        logger.info('深圳市律师协会一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.lawyers_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('深圳市律师协会公告导入完成！')
        else:
            logger.error('深圳市律师协会公告导入出现问题！')
    else:
        logger.info('深圳市律师协会没有新公告！')
