...
logger = logger_init('浙江省律师协会-数据抓取')
config = config_init()
...
def zjbar_crawler():
    ...
    prefix_url = [{'url': 'http://www.zjbar.com/cn/member/chengxinxinxi', 'origin': '浙江省律师协会'}]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('浙江省律师协会 抓取URL：' + each_url)
        # get page count
        ...
        try:
            text = base_soup.find(attrs={"class": 'g-right'}).find(attrs={"class": 'box-lxdt'}).find('script').text
            page_count = int(re.search(r'if\(pageNumber>(\d+)\)', text).group(1).strip())
        ...
        for page_num in range(page_count):
            ...
            try:
                url = each_url + '?currentPageNo=' + str(page_num) + '&.html'
                ...
                all_li = page_soup.find(attrs={"class": 'g-right'}).find(attrs={"class": 'box-lxdt'}).find_all('li')
                for index, each_result in enumerate(all_li):
                    ...
                    date = each_result.find('span').text.strip()
                    publish_date = re.search(r'[(（]([\s\S]*)[)）]', date).group(1).strip()
                    ...
                    if db.lawyers_data.find({'url': true_url}).count() == 0:
                        logger.info('浙江省律师协会新公告：' + true_url + ' title: ' + title)
                        post = {
                            'title': title,
    ...
    if len(result_list) > 0:
        logger.info('浙江省律师协会一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.lawyers_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('浙江省律师协会公告导入完成！')
        else:
            logger.error('浙江省律师协会公告导入出现问题！')
    else:
        logger.info('浙江省律师协会没有新公告！')
