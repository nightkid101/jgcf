...
def bjccgp_crawler():
    ...
    prefix_url = [
        {'url': 'http://www.ccgp-beijing.gov.cn/jdgl/wfxw/sjxzcf/', 'origin': '北京市财政局', 'type': '市级监督检查'},
        {'url': 'http://www.ccgp-beijing.gov.cn/jdgl/wfxw/qjxzcf/', 'origin': '北京市财政局', 'type': '区级行政处罚'}
    ]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('北京市财政局 ' + each_url_info['type'] + ' 抓取URL：' + each_url)
        # get page count
        ...
        try:
            text = base_soup.find(class_='a_div').find_all('a')[-2].text.strip()
            page_count = int(text)
        ...
        for page_num in range(page_count):
            ...
            try:
                ...
                all_li = page_soup.find(attrs={"class": 'guan_ul'}).find_all('li')
                for index, each_result in enumerate(all_li):
                    title = each_result.find('a').text.replace('�z', '[').replace('�{', ']').strip()
                    ...
                    date = re.search(r'\./(t|P0)(\d{4})(\d{2})(\d{2})', href)
                    publish_date = date.group(2) + '-' + date.group(3) + '-' + date.group(4)
                    if db.finance_data.find({'url': true_url}).count() == 0:
                        logger.info('北京市财政局新公告：' + true_url + ' title: ' + title)
                        post = {
                            'title': title,
    ...
    if len(result_list) > 0:
        logger.info('北京市财政局一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.finance_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('北京市财政局公告导入完成！')
        else:
            logger.error('北京市财政局公告导入出现问题！')
    else:
        logger.info('北京市财政局没有新公告！')
