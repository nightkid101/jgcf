...
def sdczt_crawler():
    ...
    prefix_url = [
        {'url': 'http://www.sdcz.gov.cn/sdczww.s?method=lanmu&cid=341', 'origin': '山东省财政厅'}
    ]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('山东省财政厅 抓取URL：' + each_url)
        # get page count
        ...
        try:
            page_count_text = base_soup.find(class_='showpage').text.strip()
        ...
        for page_num in range(page_count):
            ...
            try:
                ...
                all_li = page_soup.find(attrs={"class": 'list_info_table'}).find_all('tr')
                for index, each_result in enumerate(all_li):
                    ...
                    publish_date = each_result.find_all('td')[-1].text.strip().split(',')[-1].strip()
                    if db.finance_data.find({'url': true_url}).count() == 0:
                        logger.info('山东省财政厅新公告：' + true_url + ' title: ' + title)
                        post = {
                            'title': title,
    ...
    if len(result_list) > 0:
        logger.info('山东省财政厅一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.finance_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('山东省财政厅公告导入完成！')
        else:
            logger.error('山东省财政厅公告导入出现问题！')
    else:
        logger.info('山东省财政厅没有新公告！')
