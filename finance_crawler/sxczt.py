...
logger = logger_init('山西省财政厅-数据抓取')
config = config_init()
...
def sxsczt_crawler():
    ...
    prefix_url = [
        {
            'url': 'http://www.sxscz.gov.cn/cms_search.action?searchValue=%E5%A4%84%E7%BD%9A&classId=',
            'origin': '山西省财政厅'
        }
    ]
    for each_url_info in prefix_url:
        ...
        stop_flag = False
        logger.info('山西省财政厅 抓取URL：' + each_url)
        # get page count
        ...
        try:
            page_count_text = base_soup.find(class_='page_li_1').text.strip()
        ...
        for page_num in range(page_count):
            ...
            try:
                ...
                all_tr = page_soup.find(attrs={"class": "list_bd new_cont nianbao"}).find_all('li')
                for index, each_tr in enumerate(all_tr):
                    title = each_tr.find('a').text.strip()
                    href = each_tr.find('a')['href'].strip()
    ...
    if len(result_list) > 0:
        logger.info('山西省财政厅一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.finance_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('山西省财政厅公告导入完成！')
        else:
            logger.error('山西省财政厅公告导入出现问题！')
    else:
        logger.info('山西省财政厅没有新公告！')
