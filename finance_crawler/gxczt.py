...
logger = logger_init('广西壮族自治区财政厅-数据抓取')
config = config_init()
...
def gxczt_crawler():
    ...
    prefix_url = [
        {
            'url': 'http://www.gxcz.gov.cn/zwgk/xzcfxx/index.shtml', 'origin': '广西壮族自治区财政厅',
            'request_url': 'http://www.gxcz.gov.cn/zwgk/xzcfxx/index-'
        }
    ]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('广西壮族自治区财政厅 抓取URL：' + each_url)
        # get page count
        ...
        try:
            page_count_text = base_soup.find(class_='more-page').find_all('a')[-1].attrs['href'].strip()
        ...
        for page_num in range(page_count):
            ...
            try:
                ...
                all_li = page_soup.find(attrs={"class": 'more'}).find_all('li')
                for index, each_result in enumerate(all_li):
                    ...
                    if db.finance_data.find({'url': true_url}).count() == 0:
                        logger.info('广西壮族自治区财政厅新公告：' + true_url + ' title: ' + title)
                        post = {
                            'title': title,
    ...
    if len(result_list) > 0:
        logger.info('广西壮族自治区财政厅一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.finance_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('广西壮族自治区财政厅公告导入完成！')
        else:
            logger.error('广西壮族自治区财政厅公告导入出现问题！')
    else:
        logger.info('广西壮族自治区财政厅没有新公告！')
