...
logger = logger_init('重庆市财政局-数据抓取')
config = config_init()
...
def cqczj_crawler():
    ...
    prefix_url = [
        {
            'url': 'http://jcz.cq.gov.cn/html/xxgk/gsgg/czjd/', 'origin': '重庆市财政局'
        }
    ]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('重庆市财政局 抓取URL：' + each_url)
        # get page count
        ...
        try:
            page_count_text = base_soup.find(class_='page').text.strip()
        ...
        for page_num in range(page_count):
            ...
            if page_num == 0:
                url = each_url + 'Index.shtml'
            else:
                url = each_url + 'List_' + str(page_num) + '.shtml'
            ...
            try:
                ...
                all_li = page_soup.find(attrs={"class": 'sortlist'}).find_all('li')
    ...
    if len(result_list) > 0:
        logger.info('重庆市财政局一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.finance_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('重庆市财政局公告导入完成！')
        else:
            logger.error('重庆市财政局公告导入出现问题！')
    else:
        logger.info('重庆市财政局没有新公告！')
