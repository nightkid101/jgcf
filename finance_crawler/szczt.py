...
logger = logger_init('深圳市财政局-数据抓取')
config = config_init()
...
def szczt_crawler():
    ...
    prefix_url = [
        {
            'url': 'http://www.szfb.gov.cn/zwgk/zklyxxgk/czyjs_57120/', 'origin': '深圳市财政局'
        }
    ]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('深圳市财政局 抓取URL：' + each_url)
        # get page count
        base_page = request_site_page(each_url)
        if base_page is None:
            logger.error('网页请求错误 %s' % each_url)
            continue
        if 'GB' in base_page.apparent_encoding:
            base_page.encoding = 'GB18030'
        else:
            base_page.encoding = base_page.apparent_encoding
        base_soup = BeautifulSoup(base_page.text, 'lxml')
        try:
            page_count_text = base_soup.find(class_='list-content-page').text.strip()
        ...
        for page_num in range(page_count):
            ...
            try:
                ...
                if page_response is None:
                    ...
                    continue
                if 'GB' in page_response.apparent_encoding:
                    page_response.encoding = 'GB18030'
                else:
                    page_response.encoding = page_response.apparent_encoding
                page_soup = BeautifulSoup(page_response.text, 'lxml')
                all_li = page_soup.find(attrs={"class": 'list-content-list'}).find_all('li')[1:]
                for index, each_result in enumerate(all_li):
                    ...
                    if db.finance_data.find({'url': true_url}).count() == 0:
                        logger.info('深圳市财政局新公告：' + true_url + ' title: ' + title)
                        post = {
                            'title': title,
    ...
    if len(result_list) > 0:
        logger.info('深圳市财政局一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.finance_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('深圳市财政局公告导入完成！')
        else:
            logger.error('深圳市财政局公告导入出现问题！')
    else:
        logger.info('深圳市财政局没有新公告！')
