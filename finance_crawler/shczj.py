...
def shczj_crawler():
    ...
    prefix_url = [
        {'url': 'https://www.czj.sh.gov.cn/was5/web/search?channelid=283767', 'origin': '上海市财政局'}
    ]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('上海市财政局 抓取URL：' + each_url)
        # get page count
        ...
        try:
            text = base_soup.find(id='gjl').find_all('a')[-1].attrs['href']
            page_count = int(re.search(r'search\?page=(\d+)&channelid=283767', text).group(1).strip())
        ...
        for page_num in range(page_count):
            ...
            url = each_url + '&perpage=10&outlinepage=7&page=' + str(page_num + 1)
            ...
            try:
                ...
                all_li = page_soup.find(attrs={"class": 'table table-bordered table-striped '}).find('tbody').find_all(
                    'tr')
                for index, each_li in enumerate(all_li):
                    title = each_li.find_all('td')[1].text.strip()
                    true_url = each_li.find_all('td')[-1].find('a')['href']
                    ...
                    publish_date = each_li.find_all('td')[2].text.replace('.', '-')
                    if db.finance_data.find({'url': true_url}).count() == 0:
                        logger.info('上海市财政局新公告：' + true_url + ' title: ' + title)
                        post = {
                            'title': title,
    ...
    if len(result_list) > 0:
        logger.info('上海市财政局一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.finance_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('上海市财政局公告导入完成！')
        else:
            logger.error('上海市财政局公告导入出现问题！')
    else:
        logger.info('上海市财政局没有新公告！')
