...
logger = logger_init('湖南省财政厅-数据抓取')
config = config_init()
...
def hunanczt_crawler():
    result_list = []  # 用来保存最后存入数据库的数据
    prefix_url = [
        {'url': 'http://czt.hunan.gov.cn/xxgk/xzcf/xyxxgs/', 'origin': '湖南省财政厅'},
        {'url': 'http://czt.hunan.gov.cn/xxgk/xzcf/xzcfjdwsgk/', 'origin': '湖南省财政厅'}
    ]
    for each_url_info in prefix_url:
        each_url = each_url_info['url']
        stop_flag = False
        logger.info('湖南省财政厅 抓取URL：' + each_url)
        # get page count
        ...
        try:
            page_count_text = base_soup.find(class_='nextpage').text.strip()
            page_count = int(re.search(r'var countPage = (\d+?)//共多少页', page_count_text).group(1).strip())
        ...
        for page_num in range(page_count):
            ...
            try:
                ...
                all_li = page_soup.find(attrs={"class": 'table'}).find_all('tr')[1:]
    ...
    if len(result_list) > 0:
        logger.info('湖南省财政厅一共有%d条新公告，导入数据库中......' % len(result_list))
        r = db.finance_data.insert_many(result_list)
        if len(r.inserted_ids) == len(result_list):
            logger.info('湖南省财政厅公告导入完成！')
        else:
            logger.error('湖南省财政厅公告导入出现问题！')
    else:
        logger.info('湖南省财政厅没有新公告！')
