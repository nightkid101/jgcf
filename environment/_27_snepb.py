...
# 陕西省环境保护厅
# http://www.snepb.gov.cn/wryjg/xzcf/cfjd/index.html
# http://www.snepb.gov.cn/wryjg/xzcf/hjwf/index.html
url_format_list = [
    'http://www.snepb.gov.cn/wryjg/xzcf/cfjd/index{}.html',
    'http://www.snepb.gov.cn/wryjg/xzcf/hjwf/index{}.html',
]
...
def crawler(url_format):
    ...
    page_count_text = soup.body.find(class_='epages')
    page_count = int(re.findall(r'共(\d+)页', str(page_count_text) if page_count_text else '')[-1])
    ...
    for num in range(page_count):
        ...
        try:
            ...
            soup = bs(response.content if response else '', 'lxml')
            for x in soup.body.find(class_='suxdk').find_all('li'):
