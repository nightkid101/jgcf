...
# 宁夏回族自治区环境保护厅
# http://www.nxep.gov.cn/zwgk2/jczf/ccjc.htm
# http://www.nxep.gov.cn/zwgk2/jczf/gpdb.htm
# http://www.nxep.gov.cn/zwgk2/jczf/xzcf.htm
url_format_list = [
    'http://www.nxep.gov.cn/zwgk2/jczf/ccjc{}.htm',
    'http://www.nxep.gov.cn/zwgk2/jczf/gpdb{}.htm',
    'http://www.nxep.gov.cn/zwgk2/jczf/xzcf{}.htm',
]
gov_name = '宁夏回族自治区环境保护厅'
...
def crawler(url_format):
    ...
    page_count_text = soup.body.find(id='fanye2251')
    page_count = int(re.findall(r'\d+/(\d+)', page_count_text.text if page_count_text else '')[-1])
    ...
    for num in range(page_count):
        ...
        try:
            ...
            soup = bs(response.content if response else '', 'lxml')
            for x in soup.find(class_='news-items-r-list').find_all('li'):
                ...
                info = {
                    ...
                    'publishDate': x.em.text.strip(),
                    'url': anc_url,
                    'type': '行政处罚决定',
                    'origin': gov_name,
                    'status': 'not parsed'
                }
                if url_format == url_format_list[0] and '信息公开情况表' not in info['title']:
                    continue
