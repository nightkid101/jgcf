...
# 辽宁省环境保护厅
# http://sthj.ln.gov.cn/xxgkml/
url_format = 'http://sthj.ln.gov.cn/xxgkml/984/1276/list{}.htm'
gov_name = '辽宁省环境保护厅'
...
def crawler():
    ...
    page_count = math.ceil(
        int(re.findall(r'var m_nRecordCount = "(\d+)";', response.text if response else '')[-1]) / 20)
    ...
    for num in range(page_count):
        ...
        try:
            ...
            for x in soup.body.find_all(class_='row'):
                temp = x.find(class_='mc').a
                publish_date = str(x.find(class_='fbrq').text).replace('年', '-').replace('月', '-').replace('日', '')
                anc_url = temp.attrs['href'].strip()
