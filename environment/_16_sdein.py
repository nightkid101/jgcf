...
# 山东省环境保护厅
# http://zfc.sdein.gov.cn/xzcf/
url_format = 'http://zfc.sdein.gov.cn/xzcf/index{}.html'
gov_name = '山东省环境保护厅'
...
def crawler():
    ...
    page_count = int(re.search(r'createPageHTML\((\d+)', response.text if response else '').group(1))
    ...
    for num in range(page_count):
        ...
        try:
            ...
            for x in soup.find_all(class_='date14'):
                anc_url = x.parent.find('a').attrs['href'].strip()
                ...
                if db[collection_name].count_documents({'url': anc_url}) != 0:
                    return
                info = {
                    'title': x.parent.find('a').attrs['title'].strip(),
                    'publishDate': x.text,
                    'url': anc_url,
                    'type': '行政处罚决定',
                    'origin': gov_name,
