...
# 环保部门
# http://www.mee.gov.cn/home/rdq/jdzf/hjzf/dcdb/
url_format = 'http://www.mee.gov.cn/home/rdq/jdzf/hjzf/dcdb/index{}.shtml'
gov_name = '环保部门'
...
if config['mongodb']['dev_mongo'] == '1':
    ...
    db = MongoClient(host=config['mongodb']['mongodb_host'],port=int(config['mongodb']['mongodb_port']),
        username=None if config['mongodb']['mongodb_username'] == '' else config['mongodb']['mongodb_username'],
        password=None if config['mongodb']['mongodb_password'] == '' else config['mongodb']['mongodb_password'])[config['mongodb']['mongodb_db_name']]
...
def crawler():
    ...
    for num in range(page_count):
        ...
        try:
            ...
            soup = bs(response.content if response else '', 'lxml')
            for x in soup.body.find_all(class_='bgleft'):
