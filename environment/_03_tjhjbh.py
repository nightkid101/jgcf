...
# 天津市环境保护局
# http://hjbh.tj.gov.cn/env/environmental_monitoring/administrative_penalty_information/
url_format = 'http://hjbh.tj.gov.cn/env/environmental_monitoring/administrative_penalty_information/index{}.html'
url_prefix = 'http://hjbh.tj.gov.cn/env/environmental_monitoring/administrative_penalty_information/'
gov_name = '天津市环境保护局'
...
def crawler():
    ...
    page_count_text = soup.find(id='NPage')
    page_count = int(re.findall(r'index_(\d+)', str(page_count_text) if page_count_text else '')[-1]) + 1
    ...
    for num in range(page_count):
        ...
        try:
            ...
            soup = bs(response.content if response else '', 'lxml')
            for x in soup.find(class_='ullist').find_all('li'):
                ...
                if db[collection_name].count_documents({'url': anc_url}) != 0:
                    return
                anc_date = re.findall(r'\[(\d+)\]', x.a.text)[0] + '-' + x.span.text
                info = {
                    'title': x.a.text,
                    'publishDate': anc_date,
                    'url': anc_url,
                    'type': '行政处罚决定',
                    'origin': gov_name,
                    'status': 'not parsed'
