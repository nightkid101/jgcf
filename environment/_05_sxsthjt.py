...
# 山西省环境保护厅
# http://sthjt.shanxi.gov.cn/html/xzcfjd/index.html
url_format = 'http://sthjt.shanxi.gov.cn/html/xzcfjd/index{}.html'
url_prefix = 'http://sthjt.shanxi.gov.cn'
gov_name = '山西省环境保护厅'
...
def crawler():
    ...
    page_count_text = soup.find(class_='page-end')
    page_count = int(re.findall(r'index_(\d+)', str(page_count_text) if page_count_text else '')[-1])
    ...
    for num in range(page_count):
        ...
        try:
            ...
            soup = bs(response.content if response else '', 'lxml')
            for x in soup.find(class_='list-details').find_all('li'):
