...
# 青海省环境保护厅
# http://www.qhepb.gov.cn/hjgl/zcfg/xzfy/index{}.html
url_format = 'http://www.qhepb.gov.cn/hjgl/zcfg/xzfy/index{}.html'
gov_name = '青海省环境保护厅'
...
def crawler():
    ...
    page_count_text = soup.body.find(id='wrapper')
    ...
    for num in range(page_count):
        ...
        try:
            ...
            soup = bs(response.content if response else '', 'lxml')
            for x in soup.find(class_='single3_list').find_all('li'):

                anc_url = x.span.a.attrs['href'].strip()
                ...
                if db[collection_name].count_documents({'url': anc_url}) != 0:
                    return
                title = x.span.a.attrs['title'].strip()
                x.span.decompose()
                info = {
                    'title': title,
