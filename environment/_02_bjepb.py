...
# 北京市环境保护局
# http://www.bjepb.gov.cn/bjhrb/xxgk/ywdt/hbjczf/zlgzwfxwjds75/ede61bc1-1.html
url_format = 'http://www.bjepb.gov.cn/bjhrb/xxgk/ywdt/hbjczf/zlgzwfxwjds75/ede61bc1-{}.html'
url_prefix = 'http://www.bjepb.gov.cn'
gov_name = '北京市环境保护局'
...
def crawler():
    ...
    page_count_text = soup.find(class_='l_m_bd')
    page_count = int(re.findall(r'\d+', page_count_text.text if page_count_text else '')[-1])
    ...
    for num in range(page_count):
        ...
        try:
            ...
            soup = bs(response.content if response else '', 'lxml')
            for x in soup.find(class_='h_wz_m').find_all('li'):
