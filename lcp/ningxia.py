...
def ningxia_circ(db, logger):
    for each_circ_data in db.circ_data.find({'origin': '宁夏保监局', 'status': {'$nin': ['ignored']}}):
        ...
        if '行政处罚信息' in title:
            ...
            if document_code_compiler.search(content_text.split('\n')[0]):
                document_code = document_code_compiler.search(content_text.split('\n')[0]).group(1).strip()
            else:
                document_code = ''
            ...
            if re.search(r'.{4}年.{1,2}月.{1,3}日', publish_date_text):
                ...
                m = re.match("([0-9零一二两三四五六七八九十〇○ＯOΟ]+年)?([0-9一二两三四五六七八九十]+)月?([0-9一二两三四五六七八九十]+)[号日]?",
                             publish_date)
