...
def anhui_circ(db, logger):
    for each_circ_data in db.circ_data.find({'origin': '安徽保监局', 'status': {'$nin': ['ignored']}}):
        ...
        logger.info('安徽保监局 ' + 'Url to parse: %s' % announcement_url)
        ...
        document_code_compiler = re.compile(r'(((皖|阜阳|皖银)保监[罚筹]|[皖阜]银保监罚决字).\d{4}.\d+号)')
        if '行政处罚信息披露' in title:
            document_code = document_code_compiler.search(content_text).group(1).strip()
            if re.search(r'行政处罚信息披露.*?.(.*?).', title):
                litigant = re.search(r'行政处罚信息披露.*?.(.*?).', title).group(2).strip()
            else:
                litigant = ''
            if re.search(r'^.*?于(.*?)作出.*?处罚决定', content_text):
                publish_date_text = re.search(r'^.*?于(.*?)作出.*?处罚决定', content_text).group(1).strip()
            else:
                if re.search(r'^(.*)?，安徽.*?作出如下处罚决定：', content_text):
                    publish_date_text = re.search(r'^(.*)?，安徽.*?作出如下处罚决定：', content_text).group(1).strip()
                else:
                    publish_date_text = ''
        ...
        if '申辩' in content_text:
            ...
            if len(defense_list) != 0:
                ...
                defense_text_str = '([^。；\n]*?向.*?公告送达了《行政处罚事先告知书》.*?提出陈述申辩。|' \
                                   ...
                                   '我局向你送达《行政处罚事先告知书》（皖保监罚〔2010〕57号）后)'
                defense_compiler = re.compile(defense_text_str, re.MULTILINE)
        ...
        if db.announcement.find({'announcementTitle': title, 'oss_file_id': file_id}).count() == 0:
            ...
            logger.info('安徽保监局 数据解析 ' + ' -- 数据导入完成')
        else:
            logger.info('安徽保监局 数据解析 ' + ' -- 数据已经存在')
        db.parsed_data.update_one({'_id': file_id}, {'$set': {'parsed': True}})
        logger.info('安徽保监局 数据解析 ' + ' -- 修改parsed完成')
