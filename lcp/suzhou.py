...
def suzhou_circ(db, logger):
    for each_circ_data in db.circ_data.find({'origin': '苏州保监局', 'status': {'$nin': ['ignored']}}):
        ...
        logger.info('苏州保监分局 ' + 'Url to parse: %s' % announcement_url)
        ...
        document_code_compiler = re.compile(r'(苏州银?保监罚.\d{4}.\d+号)')
        ...
        if '申辩' in content_text:
            ...
            if len(defense_list) != 0:
                ...
                defense_text_str = '([^。；\n]*?向.*?公告送达了《行政处罚事先告知书》.*?提出陈述申辩。|' \
                                   ...
                                   '你可在收到告知书之日起10日内将陈述和申辩的书面材料提交至我局。逾期不提交视为放弃陈述权和申辩权。)'
                defense_compiler = re.compile(defense_text_str, re.MULTILINE)
        ...
        punishment_decision_text_str = r'(((依据|根据).*?第?.*?条.*?(规定)?.?我局(决定|责令|给予|于.*?向.*?发出|对.*?作出|拟对你|作出如下行政处罚)|' \
                                       r'我局经复核认为|我局决定.*?作出(如下|以下)(行政)?处罚：|' \
                                       r'根据《中华人民共和国保险法》第一百六十二的规定，我局责令|' \
                                       r'据《保险法》第一百六十一条的规定，我局决定)' \
                                       r'([\s\S]*?))' \
                                       r'(请在本处罚决定书送达之日|当事人应当在接到本处罚决定书之日|如不服本处罚决定|' \
                                       r'请(在)?接到本处罚决定书之日|如不服从本处罚决定|' \
                                       r'如对我局认定的违法事实、处罚理由及依据有异议)'
        ...
        if db.announcement.find({'announcementTitle': title, 'oss_file_id': file_id}).count() == 0:
            ...
            logger.info('苏州保监分局 数据解析 ' + ' -- 数据导入完成')
        else:
            logger.info('苏州保监分局 数据解析 ' + ' -- 数据已经存在')
        db.parsed_data.update_one({'_id': file_id}, {'$set': {'parsed': True}})
        logger.info('苏州保监分局 数据解析 ' + ' -- 修改parsed完成')
