...
def gansu_circ(db, logger):
    for each_circ_data in db.circ_data.find({'origin': '甘肃保监局', 'status': {'$nin': ['ignored']}}):
        ...
        logger.info('甘肃保监局 ' + 'Url to parse: %s' % announcement_url)
        ...
        document_code_compiler = re.compile(r'(甘银?保监罚.\d{4}.\d+号)')
        if document_code_compiler.search(content_text):
            ...
            if document_code_compiler.search(title):
                ...
                document_code = ''
            litigant_compiler = re.compile(r'^([\s\S]*?)\n' + r'(经查|经检查|依据.*?的有关规定|抽查|经抽查|个险部内勤经查|.*?公司在)')
            litigant = litigant_compiler.search(content_text).group(1).strip()
        ...
        punishment_decision_text_str = r'(((依据|根据)[^。；]*?第?[^。；]*?条[^。；]*?(规定)?\n?.?(我局)?(决定|责令|给予)|' \
                                       ...
                                       r'当事人应当在接到处罚决定书之日|当事人如对(本)?处罚决定不服|' \
                                       r'请当事人在接到本处罚决定书之日)'
        ...
        if db.announcement.find({'announcementTitle': title, 'oss_file_id': file_id}).count() == 0:
            ...
            logger.info('甘肃保监局 数据解析 ' + ' -- 数据导入完成')
        else:
            logger.info('甘肃保监局 数据解析 ' + ' -- 数据已经存在')
        db.parsed_data.update_one({'_id': file_id}, {'$set': {'parsed': True}})
        logger.info('甘肃保监局 数据解析 ' + ' -- 修改parsed完成')
