...
def wenzhou_circ(db, logger):
    for each_circ_data in db.circ_data.find({'origin': '温州保监局', 'status': {'$nin': ['ignored']}}):
        ...
        logger.info('温州保监分局 ' + 'Url to parse: %s' % announcement_url)
        ...
        document_code_compiler = re.compile(r'((温保监罚|温银保监罚决字).\d{4}.\d+号)')
        ...
        punishment_decision_text_str = r'(我局决定.*?作出.*?处罚：|处罚依据及处罚决定|' \
                                       r'根据.*?第.*?条.*?规定，现责令)' \
                                       r'([\s\S]*?)' \
                                       r'(请在本处罚决定书送达之日|当事人应当在接到本处罚决定书之日|如不服本处罚决定|' \
                                       r'请(在)?接到本处罚决定书之日|如不服从本处罚决定|\n.*?处罚的履行方式和期限|' \
                                       r'当事人应在收到本处罚决定书之日|当事人应自收到本处罚决定之日)'
        ...
        if db.announcement.find({'announcementTitle': title, 'oss_file_id': file_id}).count() == 0:
            ...
            logger.info('温州保监分局 数据解析 ' + ' -- 数据导入完成')
        else:
            logger.info('温州保监分局 数据解析 ' + ' -- 数据已经存在')
        db.parsed_data.update_one({'_id': file_id}, {'$set': {'parsed': True}})
        logger.info('温州保监分局 数据解析 ' + ' -- 修改parsed完成')
