...
# 吉林省工商行政管理局
# http://www.jlgs.gov.cn/static/xzcfxxgs/
url_format = 'http://www.jlgs.gov.cn/static/xzcfxxgs/index{}.html'
gov_name = '吉林省工商行政管理局'
...
def crawler():
    ...
    page_count_text = soup.body.find('select')
    page_count = int(page_count_text.find_all('option')[-1].text)
    ...
    for num in range(page_count):
        ...
        try:
            ...
            soup = bs(response.content if response else '', 'lxml')
            for x in soup.find(class_='contentsList-1').find_all('li'):
